...
Public Module Utils
    ...
    ''' <summary>
    ''' calculate distance between point and jpoint.
    ''' </summary>
    ...
    Public Function CalcDistBetweenPoints(X1 As Integer, Y1 As Integer, X2 As Integer, Y2 As Integer) As Integer
        Dim dist As Single
    ...
    ''' Get Y position .
    ''' </summary>
    ''' <paramname="degree">The degree between X-axis and line.</param>
    ''' <paramname="deltaX">The distance offset from Original in X-axis</param>
    ''' <paramname="deltaY">The distance offset from Original in Y-axis</param>
    ''' <paramname="Xp">The position of target point</param>
    Public Function GetYCoordinate(ByVal degree As Single, deltaX As Integer, deltaY As Integer, Xp As Integer) As Integer

        Dim b = Math.Cos(degree)
        Dim a = Math.Sin(degree)

        Dim Yp As Integer = 99999

        If Math.Abs(b) > 0.001 Then
            Yp = (0 - a * (Xp - deltaX)) / b - deltaY
        End If
    ...
    ''' <paramname="deltaY">The distance offset from Original in Y-axis</param>
    ''' <paramname="Xp">The position of target point</param>
    Public Function GetXCoordinate(ByVal degree As Single, deltaX As Integer, deltaY As Integer, Yp As Integer) As Integer
        Dim b = Math.Cos(degree)
        Dim a = Math.Sin(degree)

        Dim Xp As Integer = 99999
        If Math.Abs(a) > 0.001 Then
            Xp = (0 - b * (Yp + deltaY)) / a + deltaX
        End If
        Return Xp
    End Function
    ...
    ''' get Roi Rect.
    ''' </summary>
    ''' <paramname="scaleUnit">the unit in calibration.</param>
    ''' <paramname="CF">The calibration factor.</param>
    ''' <paramname="RoiUnit">The unit of square.</param>
    ''' <paramname="RoiSize">The size of square.</param>
    ''' <paramname="Width">The width of image.</param>
    ''' <paramname="Height">The height of image.</param>
    ''' <paramname="RoiRect">The rect to get.</param>
    Public Sub CalcRoiRect(scaleUnit As String, CF As Single, RoiUnit As String, RoiSize As Integer, Width As Integer, Height As Integer, ByRef RoiRect As RectangleF)
        Dim scaleUnitIndex, roiUnitIndex As Integer
        If scaleUnit = "m" Then
            scaleUnitIndex = 0
        ElseIf scaleUnit = "cm" Then
            scaleUnitIndex = 1
        ElseIf scaleUnit = "mm" Then
            scaleUnitIndex = 2
        ElseIf scaleUnit = "μm" Then
            scaleUnitIndex = 3
        End If

        If RoiUnit = "m×m" Then
            roiUnitIndex = 0
        ElseIf RoiUnit = "cm×cm" Then
            roiUnitIndex = 1
        ElseIf RoiUnit = "mm×mm" Then
            roiUnitIndex = 2
        ElseIf RoiUnit = "μm×μm" Then
            roiUnitIndex = 3
        End If

        Dim side = Math.Sqrt(RoiSize)
        side = side * Math.Pow(10, scaleUnitIndex - roiUnitIndex)
        Dim sideInPx = side / CF
        Dim RoiWidthF = sideInPx / Width
        Dim RoiHeightF = sideInPx / Height

        RoiRect.X = Math.Max(0.5 - RoiWidthF / 2, 0)
        RoiRect.Y = Math.Max(0.5 - RoiHeightF / 2, 0)
        RoiRect.Width = Math.Min(RoiWidthF, 1)
        RoiRect.Height = Math.Min(RoiHeightF, 1)
    End Sub
End Module
